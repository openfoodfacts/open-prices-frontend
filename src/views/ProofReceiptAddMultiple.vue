<template>
  ...
  <template v-if="step === 2">
    <v-row>
      ...
      <v-col v-if="proofUploadedList.length" cols="12" sm="6" lg="4">
        <v-card
...
export default {
  ...
  computed: {
    getReceiptAssistantProofIdsUrl() {
      return `/experiments/receipt-assistant?proof_ids=${this.proofUploadedList.map(p => p.id).join(',')}`
    },
    getUserDashboardUrl() {
  ...
  methods: {
    onProofUploaded(proof) {
      this.proofUploadedList.push(proof)
    },
