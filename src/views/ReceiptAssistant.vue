...
export default {
  ...
  computed: {
    ...
    validReceiptItems() {
      ...
      const hasInvalidDiscount = item => item.price_is_discounted && item.price_without_discount && (Number(item.price) >= Number(item.price_without_discount))
      return this.receiptItems.filter(item => item.price && isProductValid(item) && !hasInvalidDiscount(item))
    },
